...
namespace XCharts
{
    ...
    public class DirectionRadarTrainingScript : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            if (DoctorDataManager.instance.patient.trainingPlays.Count > 6) WhiteLine = "\n";
            else WhiteLine = "\n\n";
            ...
            for (int i = 0; i < DoctorDataManager.instance.patient.trainingPlays.Count; i++)
            {
                ...
                if (i == DoctorDataManager.instance.patient.trainingPlays.Count - 1)
                {
                    DirectionRadarChart.AddData(0, DoctorDataManager.instance.patient.trainingPlays[i].direction.GetDirections(), "第" + (i + 1).ToString() + "次");
                }
                // print(DoctorDataManager.instance.patient.Evaluations[i].direction.UponDirection+"+++++");

                RadarArea.Add(DoctorDataManager.instance.patient.trainingPlays[i].direction.GetRadarArea());
