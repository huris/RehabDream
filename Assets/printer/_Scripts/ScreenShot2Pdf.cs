...
namespace ShipNSea 
{
    public class ScreenShot2Pdf : MonoBehaviour
    {
        ...
        private void CreatFolder()
        {
            ...
            Directory.CreateDirectory(Application.streamingAssetsPath + "/Report/" + IntroState.username.ToString());
            picPath = Application.streamingAssetsPath + "/Report/" + IntroState.username.ToString();
            pdfPath = Application.streamingAssetsPath + "/Report/" + IntroState.username.ToString();
        }
        /// <summary>
        /// 打印调用此功能
        /// </summary>
        ...
        void SetWidthHight()
        {
            width = Screen.width / 2 - (int)((1.414 * height * .5) * .5);
            height = Screen.height;
        }
        /// <summary>
        /// 得到屏幕截图
        ...
        {
            var refunc = GetComponent<ReportPanelController>();
            refunc.ChangeAllColor();
            ...
            Texture2D tex = new Texture2D((int)(1.414 * height * .5), height, TextureFormat.RGB24, true);
            //tex.ReadPixels(new Rect(width, 0, 650, 1080), 0, 0, false);
            tex.ReadPixels(new Rect(width, 0, (int)(1.414 * height * .5), Screen.height), 0, 0, false);
